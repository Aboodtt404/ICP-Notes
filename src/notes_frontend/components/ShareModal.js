          ...
          <div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {(note.shared_with || []).length > 0 ? (
                (note.shared_with || []).map(([principal, perm]) => (
                  <div key={principal.toText()} className="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span className="text-sm font-mono truncate" title={principal.toText()}>{principal.toText()}</span>
                    <div className="flex items-center space-x-2">
                       <span className="text-sm text-gray-600">{'Read' in perm ? 'Read' : 'Write'}</span>
